...
export class UtilisateursService {
    ...
    login(utilisateur: Utilisateur) {
        return this.validateUtilisateur(utilisateur.mail, utilisateur.password).pipe(
            switchMap((utilisateur: Utilisateur) => {
                if (utilisateur) {
                    return this.authService.generateJWT(utilisateur).pipe(map((jwt: string) => jwt));
                } else {
    ...
    validateUtilisateur(mail: string, password: string) {
        return this.findByMail(mail).pipe(
            switchMap((utilisateur: Utilisateur) => this.authService.comparePasswords(password, utilisateur.password).pipe(
                map((match: boolean) => {
                    if (match) {
                        const {password, ...result} = utilisateur;
                        return result;
